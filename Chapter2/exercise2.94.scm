...
(define (greatest-common-divisor . args) (apply-generic 'gcd args))
...
(define (install-polynomial-package)
  ...
  (define (gcd-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((gcd (gcd-terms
                    (quick-sort > car (term-list p1))
                    (quick-sort > car (term-list p2)))))
          (make-poly (variable p1) gcd))
  ...
  (put 'gcd '(polynomial polynomial)
       (λ (p1 p2)
         ((transf p1 p2 'p)
          (tag (gcd-poly ((transf p1 p2 'p1) p1) ((transf p1 p2 'p2) p2))))))
  (put '=zero? '(polynomial)
       (λ (p) (or
               (eq? (cddr p) '())
