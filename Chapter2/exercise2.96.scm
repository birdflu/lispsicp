...
(define (install-polynomial-package)
  ...
  (define (div-terms l1 l2)
    ; (display l1) (newline)
    (let ((L1 (merge (quick-sort > car l1)))
          (L2 (merge (quick-sort > car l2))))
      (if (empty-termlist? L1)
  ...
  (define (gcd-terms a b)
    (define (gcd-term-integereizing a b)
      (if (empty-termlist? b)
          a
          (gcd-term-integereizing b (pseudoremainder-terms a b))))
    (let ((result (gcd-term-integereizing a b)))
      (let ((divisor (apply gcd (map (λ(x) (cadr x)) result)))) 
        (map (λ(x) (cons (car x) (list (/ (cadr x) divisor )))) result))))
  (define (integerizing-factor L1 L2)
    (let ((o1 (- (length (if (sparse-terms? L1) (terms-sparse->dense L1) L1)) 1))
          (o2 (- (length (if (sparse-terms? L2) (terms-sparse->dense L2) L2)) 1))
          (c (car (if (sparse-terms? L2) (terms-sparse->dense L2) L2))))
      ; (display o1) (newline) (display o2) (newline) (display c) (newline)
      (expt c (+ 1 (- o1 o2)))))
  (define (remainder-terms L1 L2)
    (cadr (div-terms L1 L2)))
  (define (pseudoremainder-terms L1 L2)
    (let ((int-factor (integerizing-factor L1 L2))
          (l1 (if (dense-terms? L1) (terms-dense->sparse L1) L1))
          (l2 (if (dense-terms? L2) (terms-dense->sparse L2) L2)))
      (cadr (div-terms
             (map (λ(x) (cons (car x) (list (* int-factor (cadr x))))) l1)
             l2))))
